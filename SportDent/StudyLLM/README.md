...
- `15_研究評価DBとダッシュボード.md`：保存する研究情報、人手評価、性能ダッシュボード、集計対象と制約
...
起動直後はログイン画面を表示します。ログイン後のメニューから「新規登録」で事故文入力、「DBを見る」で確定保存済みデータの一覧表示を行います。DB一覧から保存内容を編集でき、削除は確認画面とチェック操作を経て実行します。仮アカウントは次のとおりです。
...
ログイン成功後のセッションは同じブラウザで30日間保持され、ログインフォームはブラウザ標準のパスワード保存・自動入力に対応します。共用端末では利用後にログアウトしてください。

入力画面では事故状況だけを扱い、和暦・給付年度・記号は表示しません。結果画面では種別を既存DBの12候補から選択でき、歯の外傷など本文から明確に判断できる場合は候補を自動入力します。また、被災学校種に応じて有効な学年だけを選べます。学校種が未選択なら学年は無効になり、高等学校4年は登録疑義として候補から除外しています。
...
種別は既存DBの12区分に限定し、歯・眼・耳・脳神経・脊柱・胸腹部臓器・四肢と指趾・熱傷等の明確な「部位＋受傷」表現から確認候補を提示します。事故時の傷病だけでは最終的な後遺障害区分を確定できないため、候補は必ず結果画面で人が確認します。
...
公開用ユーザー名またはパスワードを変更するときも、`./scripts/install_macos_autostart.sh`を再実行します。新しいセッション署名鍵も同時に生成されるため、変更後は既存端末のログイン状態が無効になり、新しい認証情報での再ログインが必要です。
