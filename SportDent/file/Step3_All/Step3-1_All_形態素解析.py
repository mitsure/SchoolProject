...
【目的】
全データの「災害発生時の状況」をJanomeで形態素解析し、
後続の頻出語・共起語・ジャッカード解析に使う基礎CSVを作成する。

【配置】
Meikai/file/Step3_All/Step3-1_All_形態素解析.py
...
├─ Step3-1_All_解析サマリー.csv
├─ Step3-1_All_解析異常一覧.csv
└─ Step3-1_All_事例別解析文章.csv
...
【実行】
python file/Step3_All/Step3-1_All_形態素解析.py
==============================================================================
"""
...
OUTPUT_MORPHEME_CSV = OUTPUT_DIR / "Step3-1_All_形態素解析.csv"
OUTPUT_SUMMARY_CSV = OUTPUT_DIR / "Step3-1_All_解析サマリー.csv"
OUTPUT_ERROR_CSV = OUTPUT_DIR / "Step3-1_All_解析異常一覧.csv"
OUTPUT_CASE_TEXT_CSV = OUTPUT_DIR / "Step3-1_All_事例別解析文章.csv"
...
def analyze_all(
    ...
    for row_index, row in dataframe.iterrows():
        ...
        for token_number, token in enumerate(tokens, start=1):
            ...
            base_form = token.surface if token.base_form == "*" else token.base_form
        ...
        if morpheme_count == 0:
            abnormal_items.append(
                ("形態素数0", "文章はありますが形態素が抽出されませんでした。")
            )

        if len(text) <= SHORT_TEXT_LENGTH:
            abnormal_items.append(
                ("文章が短い", f"{SHORT_TEXT_LENGTH}文字以下です。")
            )

        if 0 < morpheme_count <= SHORT_MORPHEME_COUNT:
            abnormal_items.append(
                ("形態素数が少ない", f"{SHORT_MORPHEME_COUNT}形態素以下です。")
            )

        if len(text) >= LONG_TEXT_LENGTH:
            abnormal_items.append(
                ("文章が長い", f"{LONG_TEXT_LENGTH}文字以上です。")
            )

        if morpheme_count >= LONG_MORPHEME_COUNT:
            abnormal_items.append(
                ("形態素数が多い", f"{LONG_MORPHEME_COUNT}形態素以上です。")
            )

        if not analysis_words:
            abnormal_items.append(
                ("解析対象語なし", "名詞・動詞・形容詞が抽出されませんでした。")
            )
...
def create_summary(
    ...
    summary_rows = [
        ...
        (
            "解析成功率（％）",
            round(success_count / input_count * 100, 3) if input_count else 0,
        ),
        ("総形態素数", len(morpheme_df)),
        ("平均形態素数", round(case_text_df["総形態素数"].mean(), 3)),
        ("形態素数中央値", round(case_text_df["総形態素数"].median(), 3)),
        ("最小形態素数", int(case_text_df["総形態素数"].min())),
        ("最大形態素数", int(case_text_df["総形態素数"].max())),
        ("平均文字数", round(case_text_df["文字数"].mean(), 3)),
        ("文字数中央値", round(case_text_df["文字数"].median(), 3)),
        ("最小文字数", int(case_text_df["文字数"].min())),
        ("最大文字数", int(case_text_df["文字数"].max())),
        ("異常候補レコード数", len(error_df)),
        (
            "異常候補事例数",
            int(error_df["記号"].nunique(dropna=False)) if not error_df.empty else 0,
        ),
        ("処理時間（秒）", round(elapsed_seconds, 3)),
        ("ジャッカード係数", "使用しない"),
        ("次のStep", "Step3-2_All_品詞集計.py"),
    ]
...
def print_pos_preview(morpheme_df: pd.DataFrame) -> None:
    """主要品詞の件数を速報表示する。"""
    print("\n【品詞速報】")
    ...
    counts = morpheme_df["品詞"].value_counts(dropna=False)
    ...
    for pos_name in (
        ...
        print(f"{pos_name:<6}：{int(counts.get(pos_name, 0)):>10,}件")


def print_error_preview(error_df: pd.DataFrame) -> None:
    """異常候補の内訳を表示する。"""
    print("\n【解析異常候補】")
...
def main() -> None:
    ...
    try:
        ...
        require_columns(source_df, [ID_COLUMN, TEXT_COLUMN])

        morpheme_df, case_text_df, error_df = analyze_all(source_df)

        if morpheme_df.empty:
            raise ValueError("形態素解析結果が0件です。")
        ...
        save_csv(morpheme_df, OUTPUT_MORPHEME_CSV)
        save_csv(summary_df, OUTPUT_SUMMARY_CSV)
        save_csv(error_df, OUTPUT_ERROR_CSV)
        save_csv(case_text_df, OUTPUT_CASE_TEXT_CSV)

        print("\n【形態素解析結果：先頭20行】")
        print(morpheme_df.head(20).to_string(index=False))

        print_pos_preview(morpheme_df)
        print_error_preview(error_df)

        success_count = int((case_text_df["総形態素数"] > 0).sum())

        print("\n" + "━" * 78)
        print("Step3-1_All_形態素解析　正常終了")
        print("━" * 78)
        print(f"入力件数　　　　　　：{len(source_df):,}件")
        print(f"解析成功件数　　　　：{success_count:,}件")
        print(f"解析失敗件数　　　　：{len(source_df) - success_count:,}件")
        print(f"総形態素数　　　　　：{len(morpheme_df):,}件")
        print(
            f"平均形態素数　　　　："
            f"{case_text_df['総形態素数'].mean():.2f}件／事例"
        )
        print(f"最小形態素数　　　　：{int(case_text_df['総形態素数'].min()):,}件")
        print(f"最大形態素数　　　　：{int(case_text_df['総形態素数'].max()):,}件")
        print(f"異常候補レコード数　：{len(error_df):,}件")
        print(f"処理時間　　　　　　：{elapsed_seconds:.2f}秒")
        print()
        print(f"形態素解析CSV　　　　：{OUTPUT_MORPHEME_CSV}")
        print(f"解析サマリーCSV　　　：{OUTPUT_SUMMARY_CSV}")
        print(f"解析異常一覧CSV　　　：{OUTPUT_ERROR_CSV}")
        print(f"事例別解析文章CSV　　：{OUTPUT_CASE_TEXT_CSV}")
        print()
        print("同名CSVは上書きしています。")
        print("ジャッカード係数は使用していません。")
        print("次のStep：Step3-2_All_品詞集計.py")
        print("━" * 78)
