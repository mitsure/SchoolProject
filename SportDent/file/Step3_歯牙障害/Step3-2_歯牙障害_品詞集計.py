...
【出力】
Meikai/CreateData/Step3_歯牙障害/
├─ Step3-2_歯牙障害_品詞集計.csv
├─ Step3-2_歯牙障害_主要品詞集計.csv
└─ Step3-2_歯牙障害_解析サマリー.csv
...
【理由】
本Stepは品詞分布を把握する記述統計であり、
特徴間または事例間の類似度を計算する処理ではないため。
...
【実行】
python file/Step3_歯牙障害/Step3-2_歯牙障害_品詞集計.py

【次のStep】
Step3-3_歯牙障害_頻出語集計.py
==============================================================================
"""
...
def load_morpheme_csv(path: Path) -> pd.DataFrame:
    """Step3-1の歯牙障害形態素解析CSVを読み込む。"""
    if not path.exists():
        raise FileNotFoundError(
            f"入力CSVが見つかりません: {path}\n"
            "先にStep3-1_歯牙障害_形態素解析.pyを実行してください。"
        )
...
def normalize_pos_value(
    value: object,
) -> str:
    """
    品詞または品詞細分類の欠損値、空文字、アスタリスクを
    「未設定」へ統一する。
    """
    if pd.isna(value):
        return "未設定"
...
def create_analysis_summary(
    ...
    def get_pos_count(
        ...
        return int(
            main_summary.loc[
                main_summary["品詞"] == pos_name,
                "件数",
            ].sum()
        )

    noun_count = get_pos_count("名詞")
    verb_count = get_pos_count("動詞")
    adjective_count = get_pos_count("形容詞")
