...
def find_heiseimin_font() -> Path:
    """
    HeiseiMin-W3の実ファイルを検索する。
    ...
    # 1. 環境変数
    environment_path = os.environ.get("HEISEIMIN_W3_PATH")
    ...
    # 3. matplotlibが把握しているシステムフォントを確認
    for font_path in font_manager.findSystemFonts():
        try:
            ...
            if prop.get_name() == FONT_NAME:
                return Path(font_path).resolve()

        except Exception:
            continue

    raise FileNotFoundError(
        "\nHeiseiMin-W3のフォントファイルが見つかりません。\n\n"
        "文字化けしたFigureを生成しないため、処理を停止しました。\n\n"
        "次のいずれかを実施してください。\n\n"
        "方法1:\n"
        "  Meikai/font/HeiseiMin-W3.otf\n"
        "  として、利用権限のあるフォントを配置する。\n\n"
        "方法2:\n"
        "  export HEISEIMIN_W3_PATH=\"/絶対パス/HeiseiMin-W3.otf\"\n\n"
        "フォントファイル自体は、このPythonには含まれていません。"
    )
...
def configure_font() -> tuple[
    ...
    """
    HeiseiMin-W3をmatplotlibへ登録し、
    FontPropertiesを返す。
    """
    font_path = find_heiseimin_font()
    ...
    # SVG内でも文字列ではなくフォント情報を安定して保持する設定
